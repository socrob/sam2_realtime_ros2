...
class SAM2MultiNode(LifecycleNode):

    def __init__(self):
        super().__init__('sam2_multi_node')
    ...
    def on_configure(self, state: LifecycleState) -> TransitionCallbackReturn:
        try:
            self.get_logger().info('[sam2_multi_node] Configuring...')
            ...
            self.get_logger().info(f'[sam2_multi_node] Using config: {self.model_cfg}')
            self.get_logger().info(f'[sam2_multi_node] Using checkpoint: {self.checkpoint}')
            ...
            self.get_logger().info('[sam2_multi_node] SAM2 model loaded')

            return TransitionCallbackReturn.SUCCESS

        except Exception as e:
            self.get_logger().error(f"[sam2_multi_node] Exception during configuration: {e}")
            self.get_logger().error(traceback.format_exc())
            return TransitionCallbackReturn.FAILURE

    def on_activate(self, state: LifecycleState) -> TransitionCallbackReturn:
        try:
            self.get_logger().info('[sam2_multi_node] Activating...')
            ...
            self.get_logger().info('[sam2_multi_node] Subscriptions activated')
            return TransitionCallbackReturn.SUCCESS

        except Exception as e:
            self.get_logger().error(f"[sam2_multi_node] Exception during activation: {e}")
            self.get_logger().error(traceback.format_exc())
            return TransitionCallbackReturn.FAILURE

    def on_deactivate(self, state: LifecycleState) -> TransitionCallbackReturn:
        try:
            self.get_logger().info('[sam2_multi_node] Deactivating...')
            ...
            self.get_logger().info('[sam2_multi_node] Deactivated')
            return TransitionCallbackReturn.SUCCESS
        except Exception as e:
            self.get_logger().error(f"[sam2_multi_node] Exception during deactivation: {e}")
            self.get_logger().error(traceback.format_exc())
            return TransitionCallbackReturn.FAILURE

    def on_cleanup(self, state: LifecycleState) -> TransitionCallbackReturn:
        try:
            self.get_logger().info('[sam2_multi_node] Cleaning up...')
            ...
            self.get_logger().error(f"[sam2_multi_node] Exception during cleanup: {e}")
            self.get_logger().error(traceback.format_exc())
            return TransitionCallbackReturn.FAILURE

    def on_shutdown(self, state: LifecycleState) -> TransitionCallbackReturn:
        try:
            self.get_logger().info('[sam2_multi_node] Shutting down...')
            super().on_shutdown(state)
            return TransitionCallbackReturn.SUCCESS
        except Exception as e:
            self.get_logger().error(f"[sam2_multi_node] Exception during shutdown: {e}")
            self.get_logger().error(traceback.format_exc())
            return TransitionCallbackReturn.FAILURE
    ...
    def destroy_prompt_subscribers(self):
        self.destroy_subscription(self.prompt_sub)
        self.destroy_subscription(self.prompt_mask_sub)
        self.get_logger().info("[sam2_multi_node] Destroyed prompt subscribers!")

    def prompt_cb(self, msg: PromptBbox):
        self.bbox = (msg.x_min, msg.y_min, msg.x_max, msg.y_max)
        self.get_logger().info(f"[sam2_multi_node] Received bbox prompt: {self.bbox}")

    def prompt_mask_cb(self, msg: Image):
        self.mask_prompt = self.cv_bridge.imgmsg_to_cv2(msg, desired_encoding='mono8')
        self.get_logger().info("[sam2_multi_node] Received mask prompt")
        ...
        if not self.initialized:
            if self.mask_prompt is not None:
                ...
                _, self.out_obj_ids, self.out_mask_logits = self.predictor.add_new_mask(
                    frame_idx=0, obj_id=self.obj_id, mask=mask_normalized
                )
                self.get_logger().info("[sam2_multi_node] Initialized with mask prompt")
                self.mask_prompt = None
                ...
                _, self.out_obj_ids, self.out_mask_logits = self.predictor.add_new_prompt(
                    frame_idx=0, obj_id=self.obj_id, bbox=bbox_np
                )
                self.get_logger().info("[sam2_multi_node] Initialized with bbox prompt")
                self.bbox = None
                self.initialized = True
                self.obj_id+=1
            else:
            ...
            if self.mask_prompt is not None:
                ...
                _, self.out_obj_ids, self.out_mask_logits = self.predictor.add_new_prompt_during_track(
                    obj_id=self.obj_id, mask=mask_normalized, if_new_target=True
                )
                self.get_logger().info("[sam2_multi_node] Added new mask prompt")
                self.mask_prompt = None
                self.obj_id+=1
                ...
                self.predictor.add_conditioning_frame(frame)
                _, self.out_obj_ids, self.out_mask_logits = self.predictor.add_new_prompt_during_track(
                    obj_id=self.obj_id, bbox=bbox_np, if_new_target=True
                )
                self.get_logger().info("[sam2_multi_node] Added new bbox prompt")
                self.bbox = None
                self.obj_id+=1
            ...
            for i in range(0, len(self.out_obj_ids)):
                ...
                hue = (i + 3) / (len(self.out_obj_ids) + 3) * 255
